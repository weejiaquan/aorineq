...
public class UpdateCheckerTests
{
    ...
    public void ParseLatestRelease_reads_the_real_v180_response()
    {
        ...
        Assert.Equal("https://github.com/weejiaquan/aorineq/releases/download/v1.8.0/AorinEQ.exe",
            release.ExeUrl);
        Assert.Null(release.Sha256Url); // v1.8.0 predates the sha asset requirement
        Assert.Equal("https://github.com/weejiaquan/aorineq/releases/tag/v1.8.0", release.HtmlUrl);
    ...
    public void ParseLatestRelease_finds_the_sha256_asset_when_present()
    {
        ...
        Assert.NotNull(release);
        Assert.Equal("https://github.com/weejiaquan/aorineq/releases/download/v1.8.0/AorinEQ.exe.sha256",
            release!.Sha256Url);
    }
    ...
    public void ParseLatestRelease_rejects_non_https_asset_urls()
    {
        ...
        var json = GitHubReleaseFixture.LatestV180Json.Replace(
            "https://github.com/weejiaquan/aorineq/releases/download/v1.8.0/AorinEQ.exe",
            "http://evil.example/AorinEQ.exe");
        var release = UpdateChecker.ParseLatestRelease(json);
        Assert.NotNull(release);
    ...
    [InlineData("f82b23b87de02c5b5d58d57915030ca434b760a06be2c9611e735fad58851587 *AorinEQ.exe",
    ...
    public async Task FetchSha256Async_reads_the_published_digest_format()
    {
        ...
        var (listener, url, serve) = ServeText(hex + " *AorinEQ.exe\n");
