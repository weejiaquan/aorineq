...
public class ProtocolLinkTests
{
    ...
    public void Parse_full_install_skin_link()
    {
        var result = ProtocolLink.Parse(
            "aorineq://install-skin?url=https%3A%2F%2Fexample.com%2Fskins%2Fneon-bar.zip"
    ...
    {
        var result = ProtocolLink.Parse("aorineq://install-skin?url=https://example.com/cool-skin.zip");
        _out.WriteLine($"status={result.Status} link={result.Link}");
        Assert.Equal(ProtocolParseStatus.Ok, result.Status);
    ...
    {
        var result = ProtocolLink.Parse("aorineq://install-skin?url=https://example.com/my%20skin.zip");
        Assert.Equal(ProtocolParseStatus.Ok, result.Status);
        Assert.Equal("my skin", result.Link!.Name);
    }

    [Theory]
    [InlineData("aorineq://install-skin?url=http://example.com/skin.zip")]  // http downgrade
    [InlineData("aorineq://install-skin?url=file:///C:/evil/skin.zip")]     // local file
    [InlineData("aorineq://install-skin?url=ftp://example.com/skin.zip")]   // other scheme
    [InlineData("aorineq://install-skin?url=javascript:alert(1)")]          // not even a host
    ...
    {
        var result = ProtocolLink.Parse(
            "aorineq://install-skin?url=https://user:pass@example.com/skin.zip");
        _out.WriteLine($"credential url -> {result.Status}");
        Assert.Equal(ProtocolParseStatus.Malformed, result.Status);
    }

    [Theory]
    [InlineData("aorineq://install-skin")]                       // no url at all
    [InlineData("aorineq://install-skin?name=foo")]              // url missing
    [InlineData("aorineq://install-skin?url=")]                  // url empty
    [InlineData("aorineq://install-skin?url=not a url")]         // unparseable
    [InlineData("not-a-uri-at-all")]                                // garbage
    [InlineData("https://example.com/?fake=aorineq://install-skin")] // wrong scheme
    [InlineData("")]                                                // empty
    ...
    {
        var result = ProtocolLink.Parse($"aorineq://{action}?url=https://example.com/x.zip");
    ...
    {
        var result = ProtocolLink.Parse(
            "aorineq://install-skin?url=https://example.com/skin.zip&name=" + Uri.EscapeDataString(name));
    ...
    public void Parse_rejects_invalid_url_default_name()
    {
        ...
        var result = ProtocolLink.Parse("aorineq://install-skin?url=https://example.com/nul.zip");
    ...
    {
        var result = ProtocolLink.Parse(
            "aorineq://install-skin?url=https://example.com/skin.zip&sha256=" + sha);
    ...
    {
        var result = ProtocolLink.Parse(
            "aorineq://install-skin?url=https://example.com/skin.zip&sha256=" + new string('g', 64));
        Assert.Equal(ProtocolParseStatus.Malformed, result.Status);
    }
    ...
    {
        var result = ProtocolLink.Parse(
            "aorineq://install-skin?url=https://example.com/skin.zip&sha256=" + new string('A', 64));
        Assert.Equal(ProtocolParseStatus.Ok, result.Status);
        Assert.Equal(new string('a', 64), result.Link!.Sha256);
    }
    ...
    {
        var raw = "aorineq://install-skin?url=https://example.com/skin.zip&name="
            + new string('x', 5000);
        var result = ProtocolLink.Parse(raw);
    ...
    {
        var result = ProtocolLink.Parse(
            "aorineq://install-skin?url=https://example.com/skin.zip&future=stuff");
        Assert.Equal(ProtocolParseStatus.Ok, result.Status);
    ...
    public void Parse_accepts_trailing_slash_before_query()
    {
        ...
        var result = ProtocolLink.Parse("aorineq://install-skin/?url=https://example.com/skin.zip");
        Assert.Equal(ProtocolParseStatus.Ok, result.Status);
    }
    ...
    {
        var result = ProtocolLink.Parse("aorineq://install-skin/extra?url=https://example.com/skin.zip");
    ...
    [Theory]
    [InlineData("aorineq://install-skin?url=https://x.com/a.zip", true)]
    [InlineData("aorineq://install-skin?url=https://x.com/a.zip\" --evil", false)] // quote smuggling
    [InlineData("aorineq://install-skin?url=https://x.com/a b.zip", false)]        // raw space splits args
    ...
    {
        var result = ProtocolLink.Parse(
            "aorineq://apply-preset?type=eq&url=https%3A%2F%2Fexample.com%2FHD650.txt"
    ...
    {
        var result = ProtocolLink.Parse(
            "aorineq://apply-preset?type=eq&url=https://example.com/presets/HD%20650.txt");
        Assert.Equal(ProtocolParseStatus.Ok, result.Status);
    ...
    public void Parse_inline_eq_preset_link()
    {
        ...
        var raw = $"aorineq://apply-preset?type=eq&data={EqShare.Encode(preset)}&name=Warm";
    ...
    public void Inline_preset_link_without_a_name_gets_the_shared_default()
    {
        ...
        var result = ProtocolLink.Parse($"aorineq://apply-preset?type=eq&data={data}");
        Assert.Equal(ProtocolParseStatus.Ok, result.Status);
    ...
    public void Parse_reports_an_unknown_preset_type_as_needing_a_newer_version()
    {
        ...
        var result = ProtocolLink.Parse(
            "aorineq://apply-preset?type=osd&url=https://example.com/bundle.txt");
    ...
    [Theory]
    [InlineData("aorineq://apply-preset?url=https://example.com/a.txt")]          // no type
    [InlineData("aorineq://apply-preset?type=eq")]                                // no source
    [InlineData("aorineq://apply-preset?type=eq&url=http://example.com/a.txt")]   // http
    [InlineData("aorineq://apply-preset?type=eq&url=https://u:p@example.com/a.txt")] // credentials
    [InlineData("aorineq://apply-preset?type=eq&url=https://example.com/nul.txt")]   // reserved stem
    [InlineData("aorineq://apply-preset?type=eq&url=https://example.com/a.txt&scope=everything")]
    [InlineData("aorineq://apply-preset?type=eq&url=https://example.com/a.txt&sha256=abc")]
    [InlineData("aorineq://apply-preset?type=eq&data=!!!notbase64!!!")]           // bad payload
    [InlineData("aorineq://apply-preset?type=eq&data=&name=x")]                   // empty payload
    ...
    public void Parse_rejects_a_link_carrying_both_a_url_and_inline_data()
    {
        ...
        var data = EqShare.Encode(new EqPreset("x", 0, new[] { new EqBand(EqBandType.Peak, 1000, 1, 1) }));
        var result = ProtocolLink.Parse(
            $"aorineq://apply-preset?type=eq&url=https://example.com/a.txt&data={data}");
    ...
    public void Parse_rejects_a_sha256_pin_on_inline_data()
    {
        ...
        var data = EqShare.Encode(new EqPreset("x", 0, new[] { new EqBand(EqBandType.Peak, 1000, 1, 1) }));
        var result = ProtocolLink.Parse(
            $"aorineq://apply-preset?type=eq&data={data}&sha256=" + new string('a', 64));
        Assert.Equal(ProtocolParseStatus.Malformed, result.Status);
    }
    ...
    public void Parse_rejects_an_inline_preset_named_something_unusable()
    {
        var data = EqShare.Encode(new EqPreset("x", 0, new[] { new EqBand(EqBandType.Peak, 1000, 1, 1) }));
        var result = ProtocolLink.Parse(
            $"aorineq://apply-preset?type=eq&data={data}&name=" + Uri.EscapeDataString("..\\evil"));
        Assert.Equal(ProtocolParseStatus.Malformed, result.Status);
    }
    ...
    {
        var result = ProtocolLink.Parse("aorineq://autoeq?model=" + Uri.EscapeDataString("Sennheiser HD 650"));
    ...
    [Theory]
    [InlineData("aorineq://autoeq")]              // no model
    [InlineData("aorineq://autoeq?model=")]       // empty
    [InlineData("aorineq://autoeq?model=%20%20")] // whitespace only
    ...
    {
        var result = ProtocolLink.Parse(
            "aorineq://autoeq?model=" + new string('m', ProtocolLink.MaxModelLength + 1));
        Assert.Equal(ProtocolParseStatus.Malformed, result.Status);
    }
    ...
    public void Parse_rejects_control_characters_in_an_autoeq_model()
    {
        ...
        var result = ProtocolLink.Parse(
            "aorineq://autoeq?model=" + Uri.EscapeDataString("HD\n650"));
    ...
    {
        var result = ProtocolLink.Parse($"aorineq://open?page={page.ToUpperInvariant()}");
    ...
    {
        var result = ProtocolLink.Parse("aorineq://open?page=widgets");
    ...
    [Theory]
    [InlineData("aorineq://open")]
    [InlineData("aorineq://open?page=")]
