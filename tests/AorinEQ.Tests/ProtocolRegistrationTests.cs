...
{
    private readonly string _scheme = "aorineq-test-" + Guid.NewGuid().ToString("N")[..8];
    ...
    {
        Assert.False(_reg.IsRegisteredFor(@"C:\Tools\AorinEQ.exe"));
        _reg.Register(@"C:\Tools\AorinEQ.exe");
        _out.WriteLine($"registered scheme {_scheme}");
        Assert.True(_reg.IsRegisteredFor(@"C:\Tools\AorinEQ.exe"));
        ...
        Assert.Equal("\"C:\\Tools\\AorinEQ.exe\" \"%1\"", command);
        using var icon = key.OpenSubKey("DefaultIcon");
        Assert.Equal("\"C:\\Tools\\AorinEQ.exe\",0", icon?.GetValue(null));
    }

    [Fact]
    public void Register_is_idempotent_and_repoints_to_a_new_exe_path()
    {
        _reg.Register(@"C:\Old\AorinEQ.exe");
        _reg.Register(@"C:\New Path\AorinEQ.exe"); // moved exe: re-register must re-point
        Assert.False(_reg.IsRegisteredFor(@"C:\Old\AorinEQ.exe"));
        Assert.True(_reg.IsRegisteredFor(@"C:\New Path\AorinEQ.exe"));
        ...
        Assert.Equal("\"C:\\New Path\\AorinEQ.exe\" \"%1\"", command);
    }
    ...
    {
        _reg.Register(@"C:\Tools\AorinEQ.exe");
        Assert.False(_reg.IsRegisteredFor(@"C:\Elsewhere\AorinEQ.exe"));
    }
}
