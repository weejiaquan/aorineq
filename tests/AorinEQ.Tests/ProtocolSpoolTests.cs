...
public class ProtocolSpoolTests : IDisposable
{
    ...
    public ProtocolSpoolTests(ITestOutputHelper output)
    {
        ...
        _spool = new ProtocolSpool(_path, "AorinEQSpoolTest-" + Guid.NewGuid().ToString("N"));
    }
    ...
    {
        _spool.Post("aorineq://install-skin?url=https://example.com/a.zip");
        _spool.Post("aorineq://install-skin?url=https://example.com/b.zip");
        var links = _spool.TakeAll();
        _out.WriteLine("took: " + string.Join(" | ", links));
        Assert.Equal(new[]
        {
            "aorineq://install-skin?url=https://example.com/a.zip",
            "aorineq://install-skin?url=https://example.com/b.zip",
        }, links);
    ...
    public void TakeAll_skips_blank_lines()
    {
        ...
        File.WriteAllText(_path, "aorineq://x\n\n\naorineq://y\n");
        var links = _spool.TakeAll();
        Assert.Equal(new[] { "aorineq://x", "aorineq://y" }, links);
    }
    ...
    public void Post_fails_closed_when_the_spool_lock_is_held_by_another_holder()
    {
        ...
        var mutexName = "AorinEQSpoolTest-held-" + Guid.NewGuid().ToString("N");
        ...
        try
        {
            var ex = Assert.Throws<IOException>(() => spool.Post("aorineq://x"));
    ...
    public async Task Concurrent_posts_from_parallel_writers_all_land()
    {
        ...
        var tasks = Enumerable.Range(0, 20).Select(i => Task.Run(
            () => _spool.Post($"aorineq://install-skin?url=https://example.com/{i}.zip")));
