...
/// are owned by <c>App</c>'s <see cref="AorinEQ.Core.VolumeState"/> and RunAsAdmin toggle, not by
/// this window — so <c>App</c> merges just these fields into its persisted <see
/// cref="AorinEQ.Core.Settings"/> via a <c>with</c> expression rather than replacing it wholesale.
