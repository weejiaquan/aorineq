...
/// <summary>Everything <see cref="ApoWriter.RenderConfig"/> needs to produce aorineq.txt:
...
/// <summary>Owns aorineq.txt in the Equalizer APO config dir and the Include line in config.txt.</summary>
public sealed class ApoWriter : IDisposable
{
    public const string VolumeFileName = "aorineq.txt";
    ...
    /// <summary>
    /// Raised once after 5 consecutive write failures (e.g. aorineq.txt not writable),
    ...
    public void WriteConfig(EqConfigModel model)
    {
        lock (_retryLock)
        {
            ...
            if (_disposed)
                return;
            _latestModel = model;
            // Every request writes, even one that renders identically: an aorineq.txt
