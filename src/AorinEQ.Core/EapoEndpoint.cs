...
public static class EapoEndpoint
{
    ...
    /// <summary>Writes a set of values into an endpoint's FxProperties. Only ever called with
    /// <see cref="RepairFxValues"/> or a backup's own contents.
    ///
    /// It asks for SetValue and QueryValues and NOTHING ELSE, which is the whole subtlety of this
    /// method. Windows grants BUILTIN\Administrators exactly "SetValue, ReadKey" on an endpoint's
    /// FxProperties; CreateSubKey and Delete belong to Audiosrv, AudioEndpointBuilder and
    /// TrustedInstaller. The convenient spellings — CreateSubKey(..., writable: true), or
    /// OpenSubKey(..., writable: true) — both ask for KEY_WRITE, which INCLUDES CreateSubKey, so
    /// the open is refused outright by an ACL that would have permitted every write actually
    /// performed here. Measured on this machine, elevated: the failure is the requested mask, not
    /// the privilege level, and it is invisible until a real endpoint is written to.
    ///
    /// Consequently the key must already exist. It always does for a live render endpoint —
    /// AudioEndpointBuilder creates it with the endpoint — and it could not be created here
    /// anyway, since creating it needs a right on the parent that Administrators are not
    /// granted either.</summary>
    public static void WriteFxProperties(string endpointGuid, IReadOnlyList<RegValue> values)
    {
        using var baseKey = BaseKey();
        // ReadWriteSubTree names what this HANDLE may do; the rights mask names what is REQUESTED
        // from Windows. They are separate, and both are needed: the two-argument overload would
        // request the narrow mask but hand back a wrapper marked read-only (its permission check
        // is inherited from the base key), and SetValue would then fail on a handle that is in
        // fact allowed to write.
        using var key = baseKey.OpenSubKey(
                $@"{RenderRoot}\{endpointGuid}\FxProperties",
